...
public static class ActivityEndpoints
{
    public static RouteGroupBuilder MapActivityEndpoints(this WebApplication app)
    {
        ...
        // POST copy activity
        group.MapPost("/activity/copy-citizen", async (HttpRequest request, int citizenId, string dateStr,
                string newDateStr, GirafDbContext dbContext) =>
            {
                try
                {
                    var sourceDate = DateOnly.Parse(dateStr);
                    var destinationDate = DateOnly.Parse(newDateStr);

                    var activityIds = await request.ReadFromJsonAsync<List<int>>();
                    if (activityIds == null || activityIds.Count is 0)
                    {
                        return Results.BadRequest("No activity IDs provided.");
                    }
                    ...
                    var activitiesToCopy = citizen.Activities
                        .Where(a => activityIds.Contains(a.Id) && a.Date == sourceDate)
                        .ToList();

                    if (activitiesToCopy.Count is 0)
                    {
                        return Results.BadRequest("No matching activities found for the provided IDs and source date.");
                    }

                    foreach (var activity in activitiesToCopy)
                    {
                    ...
                    await dbContext.SaveChangesAsync();
                    return Results.Ok("Activities copied successfully.");
                }
                ...
                catch (Exception ex)
                {
                    ...
                    return Results.Problem($"An error occurred while copying the activities: {ex.Message}",
                        statusCode: StatusCodes.Status500InternalServerError);
                }
            })
            .WithName("CopyActivityForCitizen")
            .WithDescription("Copies a set of activities from one date to another for a citizen.")
            .WithTags("Activities")
            .Accepts<List<int>>("application/json")
            .Produces<ActivityDTO>(StatusCodes.Status201Created)
        ...
        group.MapPost("/activity/copy-grade",
                async (HttpRequest request, int gradeId, string dateStr,
                    string newDateStr, GirafDbContext dbContext) =>
                {
                    try
                    {
                        ...
                        var activitiesToCopy = grade.Activities
                            .Where(a => activityIds.Contains(a.Id) && a.Date == sourceDate)
                            .ToList();
                        ...
                        if (activitiesToCopy.Count is 0)
                        {
                            return Results.BadRequest(
                                "No matching activities found for the provided IDs and source date.");
                        }

                        foreach (var activity in activitiesToCopy)
                        {
                            grade.Activities.Add(new Activity
                            {
                                Date = destinationDate,
                                Name = activity.Name,
                                Description = activity.Description,
            ...
            .WithDescription("Copies a set of activities from one date to another for a grade.")
            .WithTags("Activities")
            .Accepts<CreateActivityDTO>("application/json")
            .Produces<ActivityDTO>(StatusCodes.Status201Created)
            .Produces(StatusCodes.Status400BadRequest)
